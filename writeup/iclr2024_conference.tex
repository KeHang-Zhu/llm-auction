...
\begin{abstract}
This paper investigates auction behavior through the len of {\em homo silicus}, simulated agents using large language models, or LLMs. 
...
Unfortunately, empirical evidence is quite expensive to generate. \cite{li2017obviously}’s OSP experiments alone, with 404 participants, cost over \$15,000.\footnote{Li mentioned that for each participant, he would pay them \$20 for participation and an additional money prize they won during the game. In total, he paid on average \$37.47 for each participant.
} The rise of LLMs raises the exciting new question as to whether there exist cheaper data generating processes that can substitute for human data for the purpose of studying human behavior, whether in economic systems or otherwise ~\cite{bubeck2023sparks, horton2023large, manning2024automated}. The present work examines this question for auctions. 

The work proceeds in four parts. 
In Section \ref{section:classic}, we seek to reproduce some classic results \avs{word smithing q: workshop ways to write this -- reproduce sounds a bit too p hacky?
What about:
"...we obtain evidence to probe classic results." ?}\cite{krishna2009auction}. 
% \dcp{kagel not a good cite for auction theory. krishna auction theory book better} 
Namely, revenue equivalence, and associated predictions about strategic play, under the first-price sealed bid (FPSB) and second-price sealed bid (SPSB) auctions in independent, private value (IPV) settings. When there are departures from the theory, we’re interested in whether the departures agree with existing empirical results \cite{kagel2020handbook}. 
...
In Section \ref{session:OSP}, we consider auctions that reduce informational and cognitive constraints for agents -- namely, ascending clock auctions. Theoretical and empirical benchmarks suggest these auctions induce more truthful play in humans, and we run experiments to test whether LLM agents also play clock auctions more truthfully than sealed bid auctions. Our specifications are inspired by \citet{li2017obviously} and \citet{breitmoser2022obviousness}, and we find that in affiliated private value (APV) settings, LLMs experimental data agrees with human behavior -- LLMs are more likely to follow the predictions of rational economic theory for ascending clock auctions. For robustness, we also reproduce these results in IPV settings. Figure \ref{fig:osp} summarizes the results, comparing the ascending second-price clock auction with a `blind' ascending clock auction (where bidders do not know when other players drop out and the auction stops at the final drop out) and the SPSB auction. 

In Section \ref{session:winner} we consider common value environments and investigate the
well-known winner’s curse. In this setting, each agent's value is the sum of a common value $c$ and a private shock $p$ (both drawn uniformly) but agents agree on the ex-post valuation of the prize (the common value $c$). This auction is more cognitively demanding than the classic IPV case as agents don't know whether high valuations are due to high draws of $c$ or $p$, and this tension produces the winner's curse: the winning agents are adversely selected to be those who least appreciated their high signal was due to a high $p$. Existing literature finds humans regularly suffer the winner's curse, and we find results vindicating this  evidence: LLMs succumb to the winner’s curse as well. 
In particular, \citet{kagel1986winner} famously find experimental evidence for the winner’s curse and argue that the winner’s curse barely bites with 3-4 bidders but seriously bites for larger auctions with 6+ bidders. Figure \ref{fig:winner} demonstrates qualitatively similar results for LLM agents. 

Finally, in section \ref{session:combinatorial}, we applied this methodology to the multiple-item auctions and provided new insights into these complex auction format that has little experiment data. We tested the difference between three popular formats in simple multi-unit combinatorial setting with two superadditive goods ($A$ and $B$): a sequential sealed-bid first-price setting where first good $A$ is auctioned, then good $B$ is auctioned; a simultaneous sealed-bid first-price setting where both good $A$ and good $B$ are auctioned at the same time; and a sealed-bid first-price setting where bidders submit bids on good $A$, good $B$, and the package $AB$. This section probes how different formats induce different bidding behavior as agents try to capture the `extra' super-additive value from obtaining the package while also hedging against the risk they only obtain one of the goods. 
...
and   any prompting language.\footnote{We will make the code-base public soon and hope this will facilitate additional empirical work.}
%
For these experiments, we ran more than $1,000$ auctions with more than $5,000$ GPT-4 agent participants for costs totaling less than $\$100$. In contrast, the largest survey of auction experiments to date comes from \citet{cox1986controlled} of $1,500+$ auctions, with total costs likely considerably higher.
...
Finally, \citet{manning2024automated} enable multiple GPT-4 agents to interact and simulate various social science scenarios, including bargaining, job interviews, and bail-setting.
...
\begin{figure}[h]
    \centering \includegraphics[width=\linewidth]{images/agent.png}
...
which maximize your profit in the long run.
To do this, you should explore many different bidding strategies, 
including possibly risky or aggressive
options for data-gathering purposes. 
Learn from the history of previous rounds
in order to maximize your total profit. 
...
When LLM agents enter the multi-round auction, a randomly drawn value for the current round will be endowed to each agent. Before asking their decisions in this round, we ask the agent to make a plan and think about strategies before making the bidding decision, to implement the Type \RNum{2} thinking in \cite{kahneman2011thinking}.
...
\begin{figure}[h]
    \centering \includegraphics[width=\linewidth]{images/ipv.png}
...
\subsubsection{Case study: source of variance}

\textbf{Under-Bidding versus Over-Bidding:}

\textbf{Second-Price versus First-Price:}
...
\begin{figure}[h]  
    \centering  
\includegraphics[width=\linewidth]{images/OSP.png}  
...
\begin{figure}[h]  % 'h' here is a placement specifier, meaning "here".
    ...
    \includegraphics[width=\linewidth]{images/cv_plot.png}  % Adjust 'scale' as necessary to resize the image.
...
\begin{figure}[h]
    \centering \includegraphics[width=0.7\linewidth]{images/learning.png}
...
Finally, we considered multi-unit combinatorial settings -- namely auctions overcomplementary goods. The settings of combinatorial auctions is very rich but complex: various applications have required vastly different designs, with fruitful applications of the theory ranging from airport time slots to course allocations to spectrum auctions (\cite{rassenti1982combinatorial}; \cite{budish2011combinatorial}; \cite{milgrom2020clock}).

These examples highlight one difficulty with combinatorial auctions (CAs) in particular as the large design space -- diverse settings require different designs, making it difficult to obtain the data necessary to test promising mechanisms before they're deployed.
...
\begin{figure}[h!]
    \centering \includegraphics[width=\linewidth]{images/Simultaneous.png}
...
\begin{figure}[h!]
    \centering \includegraphics[width=0.9\linewidth]{images/Sequ.png}
...
\begin{figure}[h]
    \centering \includegraphics[width=\linewidth]{images/menu.png}
...
\begin{figure}[h]
    \centering \includegraphics[width=0.9\linewidth]{images/social_welfare.png}
...
\avs{I think adding some actual plan output from the experiment will be cool. 

Added extra discussion for framing.}
...
Compared with experimenting with real human, LLM-agents have the benefits of reducing the cost by 3 orders of magnitude. The Lab experiment by \cite{li2017obviously} with 404 human participants costs over \$15,000, while the same experiments by LLM agents costs only \$10. Also, the original experiment had 3 phrases and cost the researchers over 3 years to iterative collect data. 

The speed of iteration and also the astoundingly low cost make much larger-scale experiments possible and also performed more often. thus, we can obtain a higher degree of statistical significance.
...
primarily putting forward a framework on how to think about LLM experimental agents as proxy for human agents.
 ...
 will use our code to run simulations testing their own prompt variations. \avs{Note for John: is the conclusion sexy enough?}
...
\begin{figure}[h]
    \centering \includegraphics[width=\linewidth]{images/FPSB_off-box.png}
