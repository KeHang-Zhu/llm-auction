...
This means that, regardless of what other bidders do, it is always in a bidder's best interest to bid their true valuation for the item being auctioned. Hence, we say the SPSB is dominant strategy incentive compatible and can express the equilibrium bidding strategy in an SPSB auction as:
...
Note that, compared with the dominant strategy in SPSB, this equilibrium holds only under certain assumptions, such as the common uniform distribution of valuations and risk-neutrality. While the distribution for values is common and known to LLM agents, we exert no control over their risk preferences.
...
\subsubsection{New Empirics: Intervention}
So basically all of the under bidding is due to risk aversion.
Suggests if we prompt them — don’t be risk-averse — we can get great behavior.
So, we did a intervention for the First-Price Sealed-Bid to leverage the risk-aversion level prompting.

\begin{quote}
\begin{lstlisting}[basicstyle=\ttfamily]
Some advice: Do NOT be afraid of risk. 
All you care about is maximizing expected profit. 
For example, a 50/50 coin flip between $0 and $101 
