...
environment (a stylized eBay marketplace) that serves validation only. A
supplementary school-choice matching experiment (student-proposing deferred
acceptance), used to unbundle the menu description, is described and reported
...
\subsubsection{Supplementary matching experiment}\label{sec:design-da}

A supplementary school-choice matching experiment---student-proposing deferred
acceptance in a $4\times4$ market with Ergin-acyclic priorities
\citep{ergin2002efficient,ashlagi2018stable}, in which LLMs play the role of
students submitting rank-order lists---probes the descriptions lever outside the
auction laboratory, unbundling the menu description into its invariance content
and its mechanics. Because it is not a co-equal domain, we defer its full
specification---environment, priority orders, prompt skeletons, elicitation
protocol, and results---to Appendix~\ref{app:da}, which reports it end to end.
...
Matching (DA)$^{f}$ & direct (menu descriptions) & $v_{i,s}=c_s+\varepsilon_{i,s}$;\; $c_s\sim U[40,70]$, $\varepsilon_{i,s}\sim U[0,20]$ & rank-order list & $4\times4$ & 50 & canonical four \\
\bottomrule
\end{tabular}}
\par\smallskip
...
and gemini-2.5-flash (full lever grid), Appendix~\ref{app:ablations}. Gemma ran the
...
$^{f}$\,Supplementary matching experiment (menu-description split): design
summary in Section~\ref{sec:design-da}; implementation details and results
...
with the auction domain. Family~A (extensive form) is implemented
as a change of \emph{mechanism}: the ascending clock replaces the sealed-bid
interface, with value draws, allocation and payment rules, model, and
temperature all held fixed. Families~B and~C are implemented as \emph{prompt
treatments} on the sealed-bid baseline: each treatment modifies
...
Two sub-families scaffold a mode of reasoning without revealing the dominant
...
in the auction---telling the agent \emph{what happens} under each action, not
what to do. \textbf{C3, belief formation} (first- and second-order beliefs): the
prompt directs attention to what opponents will do, or to what opponents think
the agent will do. (Forward planning, C2 in the taxonomy of
Section~\ref{sec:framework}, has no axis in a one-shot sealed bid and therefore
does not enter the auction grid.)
...
auctions still unfold over multiple within-instance steps, but
...
\begin{enumerate}
    ...
    increase/hold decision (eBay), or a rank-order list (direct DA).
    ...
    with the true school priorities.
\end{enumerate}
...
only where pairwise sign tests across the model cells support
...
(e.g., contingent scaffolds versus safety descriptions) are
...
perfection. Concordance of the lever ordering across
model cells (Kendall's $W$ with bootstrap confidence intervals),
...
Knife-edge cells (e.g., near-zero error rates under the safety descriptions)
are topped up to $K=100$ before adjacent rungs are declared
separated.
