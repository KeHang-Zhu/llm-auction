...
\TODO{Provenance (plan \S5.5, E2): the cross-model clock cells currently
reported in Section~\ref{sec:ranking} reproduce from the imported pipeline,
whose clock configurations are APV (common component $[0,29]$) with open and
blind variants and mixed increments (\$1 vs.\ \$0.5)---not the IPV clock the
predecessor draft describes. The harmonized IPV-clock re-run
(four models, $K=50$) will finalize the identification cell.}
...
(2024-10-22), Gemini 2.0 Flash, Gemma 27B; appendix tiers = gpt-5-mini
(frontier tier) and Llama-3-8B (capability floor),
Appendix~\ref{app:ablations}. \TODO{E4: Gemma on the fidelity battery
pending.}
$^{c}$\,\TODO{Current clock cells ran $K=40$ (configs \texttt{repetitions:
40}), below the pinned $K=50$; top up in the harmonized re-runs.}
$^{d}$\,\TODO{E2: IPV clock $\times$ four models pending; the imported
pipeline's clock cells are APV with mixed increments (\$1 vs.\ \$0.5), not the
IPV clock described in the predecessor draft.}
