...
\citep{li2024designing}. Our two domains are chosen to make this lever space
fully operable. The SPSB auction and student-proposing deferred acceptance (DA)
are both strategy-proof, and both are \emph{strategically simple} in the sense
of \citet{borgers2019strategically}---the dominant strategy is independent of
beliefs entirely---yet neither direct form is obviously strategy-proof, and
both admit OSP implementations: the ascending clock for the auction
\citep{li2017obviously}, and, under Ergin-acyclic priorities
\citep{ergin2002efficient}, an iterative query protocol for DA
\citep{ashlagi2018stable}. That wedge between strategy-proofness on paper and
obviousness in play is the identifying variation the levers act on. The
division of labor between the domains is fixed up front: auctions are the
paper's main domain, and every lever family is evaluated there in the main
text (Section~\ref{sec:ranking}); the matching domain replays the full grid as
a robustness and generalization check, reported in Appendix~\ref{app:da} and
cited in the main text where it sharpens an auction result.
...
backward induction over the price trajectory. Our Family~A levers are the
ascending clock (in open AC and closed AC-B variants; Section~\ref{sec:design})
and the iterative OSP implementation of DA, in which applicants answer a
sequence of accept/reject queries rather than submitting a rank-order list
(\emph{iterative DA}, versus one-shot \emph{direct DA}). To our knowledge, no
prior experiment---human or artificial---has tested the OSP implementation of
DA; Appendix~\ref{app:da} reports the first.
...
nothing), in both domains. It says what \emph{happens} under each action, never
...
over---so C2 scaffolds are deployed only in the matching domain (results in
Appendix~\ref{app:da}), where the DA algorithm processes the
submitted ranking through multiple internal rounds of proposals and rejections,
giving lookahead a natural (if, under strategy-proofness, strategically
irrelevant) place to bind. We test one-step and two-step \emph{lookahead}
prompts that invite the agent to simulate rejection chains before reporting.
\textbf{(C3) Belief formation}: since both mechanisms are strategy-proof,
optimal play is independent of beliefs about opponents---this is precisely
strategic simplicity \citep{borgers2019strategically}. First-order (``what do
...
Iterative DA (A) & DA & \citet{ashlagi2018stable}; \citet{ergin2002efficient} & Largest improvement \\
\addlinespace
Menu restatement (B1) & Auction, DA\textsuperscript{a} & \citet{gonczarowski2023strategyproofness} & Weak/none (human behavioral null) \\
Payoff / Rejection Safety (B2) & Auction; DA & \citet{gonczarowski2023strategyproofness}; \citet{vickrey1961counterspeculation} & Improvement, approaching OSP \\
Clock-framing (B3) & Auction & \citet{breitmoser2022obviousness} & Improvement, approaching OSP \\
\addlinespace
Payoff Tree (C1) & Auction, DA & \citet{li2024designing} & Improvement (axis binds) \\
Lookahead, 1- and 2-step (C2) & DA only (appendix) & \citet{pycia2023theory} & Irrelevant at best; backfire risk \\
Beliefs, 1st/2nd order (C3) & Auction, DA & \citet{borgers2019strategically} & Inert if dominance understood; any effect diagnoses mis-strategizing \\
\addlinespace
Risk / loss framings (D) & Auction, DA & \citet{kahneman1979prospect} & Payoff-irrelevant; inert for a rational agent (Appendix~\ref{app:prospect}) \\
\bottomrule
\end{tabular}
\caption{The lever space. Every lever holds the outcome rule fixed; families differ in what else they hold fixed (Family A changes the extensive form; B changes only the description; C changes only the invited reasoning; D changes only the induced preferences). Predictions are stated ex ante from the cited theory. Auction cells are reported in the main text (Section~\ref{sec:ranking}); matching cells are the generalization check (Appendix~\ref{app:da}). Verbatim prompt texts and the old-to-new label mapping appear in Appendix~\ref{app:prompts}. \textsuperscript{a}\,In the matching domain the menu description is deployed in two variants---invariance-property and mechanics-only---to separate the menu format from its invariance content (Appendix~\ref{app:da}).}
...
demand actually binds. Concretely: (i) C1 (contingent reasoning) should help in
both domains, because case-by-case reasoning over opponent profiles is the core
demand of both non-OSP formats \citep{li2024designing}; (ii) C2 (forward
planning) has no axis to relax in a one-shot sealed bid and, in DA, scaffolds
reasoning about algorithm dynamics that strategy-proofness renders irrelevant;
(iii) C3 (beliefs) should be exactly inert, because dominant strategies are
belief-free \citep{borgers2019strategically}. The diagnostic content of H3 is
in its failure modes. Any measured effect of a belief scaffold---in either
direction---is evidence that the agent is not playing the dominant strategy but
treating the interaction as a Bayesian game, forming beliefs about competitors
and best-responding to them. Likewise, scaffolds that activate strategic
simulation without resolving it (lookahead in DA, beliefs anywhere) hand the
agent material to strategize with; if the agent has not internalized
strategy-proofness, we predict such scaffolds \emph{backfire}, and most
severely for the weakest reasoners. Backfiring is thus not an embarrassment for
