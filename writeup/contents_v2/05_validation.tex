...
\subsection{Field Microstructure: eBay Closing Rules}\label{sec:validation-ebay}

The validations above are anchored to laboratory data; our final behavioral check is anchored to the field. A hallmark of hard-close online auctions is \emph{bid sniping}: bidders delay their bids until the final moments of the auction to avoid triggering competitive responses. Sniping is widely observed on eBay and has long been studied as a behavioral and strategic departure from the simple ``bid-your-value'' logic implied by proxy bidding \citep{roth2002last}; it is potentially costly from a design standpoint because it compresses information revelation into the auction's final moments. The classic remedy is a \emph{soft-close} (auction-extension) rule: \citet{roth2002last} and \citet{ockenfels2006late} document far less sniping on early Amazon.com auctions (soft close) than on eBay (hard close), with Amazon bids spread throughout the auction. We implement a simplified eBay-style proxy-bidding environment---each bidder submits a maximum bid and the platform bids on her behalf at the second-highest bid plus an increment, giving the flavor of an open second-price auction---and vary the closing rule and the presence of a hidden reserve across four treatments (procedures in Appendix~\ref{app:ebay-process}).

LLM bidders respond to these temporal incentives in the same direction as the field evidence. We measure the \emph{final-winning-bid time}: the last period in which the eventual winner changes her maximum bid, i.e., when the winner is effectively determined. Under the standard hard-close rule, GPT-4o bidders concentrate their decisive bid updates at the deadline: $63.3\%$ of winners in the baseline treatment place their final maximum-bid change on the scheduled last day ($50.0\%$ pooling the hard-close treatments), producing the deadline-spiked histogram in the left panel of Figure~\ref{fig:ebay-sniping}. Introducing a soft-close extension rule sharply attenuates this last-period concentration: the last-day share collapses to $7.6\%$, the distribution of final-winning-bid times shifts earlier (mean day $5.0$ versus $6.7$; Mann--Whitney $p = 3.5\times10^{-5}$, Kolmogorov--Smirnov $p = 7.2\times10^{-6}$), and a quarter of soft-close winners place their decisive bid on the \emph{first} day---bid-early-and-hold. Soft close de-incentivizes waiting and spreads competition earlier in time---precisely the mechanism emphasized in the human empirical literature. Both soft-close treatments show this attenuation relative to the hard close; layering a hidden reserve on top of the soft close does \emph{not} dampen last-period activity further (treatment-level detail in Appendix~\ref{app:ebay}). The companion hidden-reserve treatments show only limited behavioral and revenue sensitivity, consistent with the large-sample field evidence of \citet{einav2011learning}; we report those null results in Appendix~\ref{app:ebay}---together with a reserve audit showing that the simulated proxy-price rule never lifts the closing price to a met reserve, so the revenue null partly reflects a mechanical channel being disabled rather than bidder insensitivity alone---and a \TODO{Myerson-level reserve check (plan §6, nice-to-have)}.

\begin{figure}[ht]
    \centering
    \begin{subfigure}[t]{0.495\linewidth}
        \centering
        \includegraphics[width=\linewidth]{ebay-winning-bid-non-closing.png}
        \caption{Hard close (T1 $+$ T3 pooled, $N=120$).}
        \label{fig:ebay-sniping-hard}
    \end{subfigure}\hfill
    \begin{subfigure}[t]{0.495\linewidth}
        \centering
        \includegraphics[width=\linewidth]{ebay-final-winning-closing.png}
        \caption{Soft close (T2 $+$ T4 pooled, $N=119$).}
        \label{fig:ebay-sniping-soft}
    \end{subfigure}
    \caption{\textbf{Final-winning-bid timing in simulated eBay-style auctions, IPV environment: hard close vs.\ soft close.} Final-winning-bid time is the last period in which the eventual winner changes her maximum bid. Under the standard (hard-close) rule, decisive bids concentrate on the scheduled final day (day 9: $63.3\%$ in the baseline treatment T1, $50.0\%$ pooled across hard-close treatments); under the soft-close (auction-extension) rule the day-9 share falls to $7.6\%$ and the distribution shifts substantially earlier (Mann--Whitney $p = 3.5\times10^{-5}$; Kolmogorov--Smirnov $p = 7.2\times10^{-6}$). Per-treatment CDFs appear in Appendix~\ref{app:ebay} (Figure~\ref{fig:ebay-cdf}).}
    \label{fig:ebay-sniping}
\end{figure}
...
\addlinespace
Sniping / closing-rule response & Late bidding under hard close (eBay); attenuated under soft close (Amazon) \citep{roth2002last,ockenfels2006late} & Decisive bids concentrate at the deadline under hard close (63.3\% final-day share in T1); soft close collapses the share to 7.6\% (MW $p=3.5\times10^{-5}$; Fig.~\ref{fig:ebay-sniping}) & Transfers ($\checkmark$) \\
\addlinespace
