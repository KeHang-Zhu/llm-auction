...
Gemma, and both are recorded cell by cell in the replication file. The direct-revelation
...
on the mislabeled cells. The matching cells for Claude 3.5 Haiku and Gemini
...
the levers, Kendall's $W$ is $0.70$ in auctions ($p = 0.004$), $0.90$ in the
matching domain ($p = 4 \times 10^{-4}$), and $0.81$ pooled over the eight
cells and seven common levers ($p = 8.4 \times 10^{-7}$): the lever ordering
is far from independent across models and domains, with the matching domain
nearly unanimous. (These statistics are computed on the corrected baselines
described below; correcting the contaminated axis-2 ``baseline'' cell
\emph{raised} auction concordance from $0.43$ to $0.70$---the mislabeled
...
\paragraph{The frontier: the ranking compresses, and parts of it invert.}
...
$p < 0.001$), while leaving gpt-5 and gpt-5-mini exactly truthful. A
description with no incentive content can still break an otherwise perfect
strategic reasoner, and which reasoner it breaks is not predictable from
baseline capability. Third, \emph{the top rung inverts}: the ascending
clock, which improves every incumbent model, makes frontier play
\emph{worse} than its own sealed baseline (Claude Sonnet 5: SMAD $0.27\%$
sealed vs.\ $8.6\%$ clock, $p<0.001$; gpt-5 and gpt-5-mini shift by
$+0.3$--$0.6$, partly tick granularity)---once the cognitive problem is
solved, the extensive-form lever's residual effect is interface friction,
not simplification. The design lesson for mixed fleets is asymmetric:
