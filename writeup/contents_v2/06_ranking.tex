...
mechanism but asserting the invariant that makes truth-telling safe.
...
analysis qualify individual cells without moving any rung. First, Gemma's
auction axis-2 baseline is anomalously good (SMAD $8.6\%$ vs.\ $20.7\%$
pooled across its axes), so Gemma's negative auction marks for the safety,
tree, and lookahead levers all flip sign when computed against the pooled
baseline; we treat them as baseline noise, not lever backfires. Second,
...
3$^{*}$ & Menu, invariance property (DA) & B1 & menu + invariant & --- & 1.8 & $+0.65$ [$-0.52, +0.95$] & --- (lab prediction) \\
...
3$^{*}$ & Menu, mechanics only (DA) & B1 & menu, computation restated & --- & 6.9 & $-1.67$ [$-5.01, +0.57$] & --- (lab prediction) \\
\midrule
...
a \emph{double dissociation} between stated reasoning and behavior
...
flat relative to baseline ($3.5\%$ vs.\ $2.8\%$, $p = 0.66$); the plans keep
the same shading rhetoric and simply shrink the shade to pennies (``I will
bid \$31.9 to avoid overbidding,'' value \$32). The \emph{worst-case}
scaffold (C1) does the opposite: it massively shifts stated reasoning
(worst-case language rises from $2\%$ to $43\%$) while bids barely move. The
\emph{Payoff Tree} moves both, and the \emph{menu} restatement moves
neither, serving as the placebo row. A classical mediation analysis
therefore fails at the first stage: whatever Payoff Safety does, it does not
work by making agents verbalize why truth-telling is safe. Stated
comprehension and behavioral improvement are separable outcomes, and our
effective descriptions improve the latter without the former---which is why
we describe them as changing behavior, not stated understanding, and why the
traces should not be read as faithful mediators of the lever effects.
...
(pooled over four models, sealed-bid second-price cells; cluster-robust tests
in \texttt{results/traces/ols\_results.txt}). Payoff Safety improves bids
without changing what agents say; the worst-case scaffold changes what agents
say without improving bids. Baselines are the axis-specific baseline cells of
each lever's own grid, per the convention of Section~\ref{sec:ranking}
(hence the small variation across rows); under the dedicated-SPSB convention
