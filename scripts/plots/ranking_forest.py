...
gpt-5-mini has NO intervention cells anywhere in the repo (verified by the
auction cell builder; recovered_logs/experiment_logs_with_explanation is
gpt-4o in every config), so no gpt-5-mini series appears.
"""
...
LEVERS = [
    ...
    ("safety", "B2", "Safety description (payoff / rejection)",
     "axis2_forward_onestep", "axis2_forward_baseline",
    ...
    ("tree", "C1", "Payoff tree (contingent)",
     "axis2_forward_tree", "axis2_forward_baseline",
     "axis1_tree", "direct_baseline"),
    ("lookahead", "C2", "Forward planning (lookahead)",
     "axis2_forward_backward_induct", "axis2_forward_baseline",
...
def draw(tab, pooled, anchors, tau_mat):
    ...
    fig.text(0.03, 0.006,
             "Auction baselines: axis-specific where available, else pooled axis baseline; DA baseline: direct_baseline. "
             "OSP-DA cells have tau = 0 by construction (censored). gpt-5-mini: no intervention cells exist.",
             fontsize=6.2, color=MUTED)
...
def write_concordance(tab, pooled, anchors, W_res, signs, tau_mat, piv,
    ...
    L.append("Generated by `scripts/plots/ranking_forest.py` (numpy seed "
             ...
             "reconstructions. `frontier_cells.csv` was NOT available (the "
             "frontier agent failed); gpt-5-mini has no intervention cells "
             "in the repo, so no frontier series appears anywhere below.\n")
...
def main():
    ...
    viol.append(
        f"4. **Gemma auction axis-2 baseline anomaly**: Gemma's "
        f"axis2_forward_baseline SMAD (8.6%) is far below its other "
        f"baselines (axis1 29.1%, axis3 24.5%, pooled 20.7%), so every "
        f"Gemma axis-2 auction rho (safety {rho_of('safety', 'google/gemma-3-27b-it', 'auction'):+.2f}, "
        f"tree {rho_of('tree', 'google/gemma-3-27b-it', 'auction'):+.2f}, "
        f"lookahead {rho_of('lookahead', 'google/gemma-3-27b-it', 'auction'):+.2f}) "
        f"flips sign under the pooled baseline (see sensitivity list below). "
        f"Treat Gemma's negative safety/tree auction marks as "
        f"baseline-noise-driven, not as lever backfires.")
    viol.append(
    ...
    sens = ("## Tier violations and within-tier flips (honest statement)\n\n"
            ...
            "Auction rho uses the canonical axis-specific baselines. Cells "
            "whose rho sign flips under the pooled axis baseline:\n\n"
